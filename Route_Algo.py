...
def calculate_all_distances(Jobs, Workers):
    ...
    all_distances = zeros(dimension, dimension)
...
def residual(G, x):
    ...
    Gf = []
    for u in range(len(G)): Gf.append(dict())
...
def aug_paths(G, s, t, x):                          # Walk the graph from node s, x is a flow
    ...
    while Q:                                        # Still nodes to visit
        ...
        try: F[u[0]] = min(F[u[1]], Gf[u[1]][u[0]]) # Update flow label
        except: F[u[0]] = inf                       # u[0] = s?, flow transported to the predessor is inf (no predecessor)
        for v in Gf[u[0]]:
